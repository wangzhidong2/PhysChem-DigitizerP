    ...
    def update_stats(self):
        ...
        for unit in self.panel.units:
            ...
            parts = [f"{unit.label}: 数据点 {n}",
                     f"电压 平均={v_arr.mean():.4f}V 最大={v_arr.max():.4f}V "
                     f"最小={v_arr.min():.4f}V",
                     f"电流 平均={unit.format_current(float(i_arr.mean()))}{unit_label} "
